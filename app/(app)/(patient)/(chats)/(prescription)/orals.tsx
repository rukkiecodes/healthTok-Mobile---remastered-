...
export default function orals () {
  ...
  const fetchPresriptions = async () => {
    try {
      ...
      const q = query(collection(db, "patient", String(auth.currentUser?.uid), 'presriptions', uid, 'presriptions'), where('prescriptionType', '==', 'Tablet'), orderBy("timestamp", "desc"));
